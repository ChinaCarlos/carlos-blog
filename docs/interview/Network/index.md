...
![http 请求头](https://raw.githubusercontent.com/ChinaCarlos/carlos-blog/main/docs/interview/images/head.png)
![http 响应头](https://raw.githubusercontent.com/ChinaCarlos/carlos-blog/main/docs/interview/images/response.png)
![http 响应头1](https://raw.githubusercontent.com/ChinaCarlos/carlos-blog/main/docs/interview/images/response1.png)
...
## 21. 响应头和返回头 都有什么常见的属性

### 1. `Content-Type`

- **定义**：指定返回数据的 MIME 类型（媒体类型），告诉客户端如何处理响应内容。
...
- **作用**：用于告知客户端响应体的长度，客户端可以根据这个值来判断响应是否完整。

### 3. `Cache-Control`

- **定义**：用于指定缓存策略，控制资源的缓存行为。
- **示例**：`Cache-Control: no-cache, no-store, must-revalidate`
- **常见值**：
  - `no-cache`：强制缓存必须重新验证（即使缓存仍然有效）
  - `no-store`：不缓存响应内容
  - `max-age=seconds`：设置缓存的最大生存时间（单位：秒）
  - `public`：响应可被任何缓存存储
  - `private`：响应仅能被客户端缓存，不能被共享缓存存储

### 4. `Date`

- **定义**：指定响应发送的日期和时间。
- **示例**：`Date: Wed, 21 Oct 2020 07:28:00 GMT`
- **作用**：帮助客户端了解服务器响应的时间，有助于处理缓存和同步操作。

### 5. `ETag`

- **定义**：资源的唯一标识符，用于判断资源是否修改。
- **示例**：`ETag: "686897696a7c876b7e"`.
- **作用**：与缓存控制配合使用，客户端可以发送 `If-None-Match` 请求头来验证是否需要下载资源。

### 6. `Last-Modified`

- **定义**：指示资源的最后修改时间。
- **示例**：`Last-Modified: Tue, 15 Nov 2020 12:45:00 GMT`
- **作用**：允许客户端在发起请求时使用 `If-Modified-Since` 头部来检查资源是否自上次访问后发生了变化。

### 7. `Location`

- **定义**：用于指示客户端应该转到的新的 URL 地址。
- **示例**：`Location: https://www.example.com/redirected-page`
- **作用**：通常用于 3xx 重定向响应，指示客户端新的位置。

### 8. `Set-Cookie`

- **定义**：用于向客户端设置 cookie。
- **示例**：`Set-Cookie: sessionId=abc123; Path=/; HttpOnly; Secure`
- **作用**：在客户端存储服务器传递的 cookie 信息，用于会话管理、认证等。

### 9. `Access-Control-Allow-Origin`

- **定义**：用于指定允许跨域请求的源。
- **示例**：`Access-Control-Allow-Origin: *`
- **作用**：如果支持跨域资源共享（CORS），服务器会通过这个头部指定允许哪些源进行跨域请求。

### 10. `X-Content-Type-Options`

- **定义**：指示浏览器不要根据内容来自动推断 MIME 类型。
- **示例**：`X-Content-Type-Options: nosniff`
- **作用**：用于防止一些类型的攻击，如 MIME 类型混淆攻击。

### 11. `Content-Encoding`

- **定义**：指示服务器返回的响应数据的压缩方式。
...
- **定义**：指定当前响应的 Web 服务器的名称或版本。
- **示例**：`Server: Apache/2.4.1 (Unix)`
- **作用**：告知客户端服务器的具体信息，通常用于服务器标识和版本信息。

### 13. `Vary`

- **定义**：指定响应内容是根据请求的某些头部字段来变化的。
- **示例**：`Vary: Accept-Encoding`
- **作用**：用于优化缓存策略，根据请求的不同头部来提供不同的响应内容。

### 14. `X-Frame-Options`

- **定义**：控制是否允许页面被嵌入到 iframe 中。
- **示例**：`X-Frame-Options: DENY`
- **常见值**：
  - `DENY`：不允许页面被嵌入到 iframe 中
  - `SAMEORIGIN`：仅允许同源网站嵌套

### 15. `Strict-Transport-Security (HSTS)`

- **定义**：指示浏览器仅通过 HTTPS 协议访问该站点。
- **示例**：`Strict-Transport-Security: max-age=31536000; includeSubDomains`
- **作用**：用于增强 HTTPS 的安全性，防止中间人攻击。

### 16. `Transfer-Encoding`

- **定义**：指示数据的传输编码方式。
- **示例**：`Transfer-Encoding: chunked`
- **作用**：当响应体非常大时，服务器可以通过分块传输编码逐步发送数据，客户端可以立即处理部分数据。

### 17. `X-XSS-Protection`

- **定义**：启用或禁用浏览器的 XSS 防护功能。
...
| **Access-Control-Allow-Origin** | 支持跨域资源共享（CORS）           |
| **X-Content-Type-Options**      | 防止浏览器自动推断 MIME 类型       |
| **Content-Encoding**            | 表示响应体的压缩类型               |
| **Server**                      | 服务器信息                         |
| **Vary**                        | 指示响应内容与请求头部某些字段相关 |
| **X-Frame-Options**             | 防止页面被嵌入到 iframe 中         |
| **Strict-Transport-Security**   | 强制通过 HTTPS 协议访问            |

这些响应头字段帮助服务器与客户端之间更有效地管理资源、优化性能、提高安全性以及增强用户体验。
